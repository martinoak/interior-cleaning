...
enum InvoiceTypes: string
{
    ...
    {
        return match ($type) {
            'T' => '<span class="bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100 rounded-full px-3 py-1 text-xs font-semibold">'.self::EARNING->value.'</span>',
            'V' => '<span class="bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100 rounded-full px-3 py-1 text-xs font-semibold">'.self::EXPENSE->value.'</span>',
