            ...
            <div class="flex items-center space-x-16">
                @if($vehicle->stk)
                    <p class="font-normal text-gray-700 dark:text-gray-400">
                        ...
                        <span class="badge @if($vehicle->stk->diffInDays(now()) > -14) badge-red @elseif($vehicle->stk->diffInDays(now()) > -60) badge-orange @endif">{{ $vehicle->stk->format('j.n.Y') }}</span>
                    </p>
                @endif
                @if($vehicle->type === \App\Enums\VehicleType::CAR->value && $vehicle->oilChange)
                    <p class="font-normal text-gray-700 dark:text-gray-400">
                        ...
                        <span class="badge @if($vehicle->oilChange->diffInDays(now()) > -14) badge-red @elseif($vehicle->oilChange->diffInDays(now()) > -60) badge-orange @endif">{{ $vehicle->oilChange->format('j.n.Y') }}</span>
                    </p>
                @elseif($vehicle->type === \App\Enums\VehicleType::TRUCK->value && $vehicle->tachograph)
                    <p class="font-normal text-gray-700 dark:text-gray-400">
                        ...
                        <span class="badge @if($vehicle->tachograph->diffInDays(now()) > -14) badge-red @elseif($vehicle->tachograph->diffInDays(now()) > -60) badge-orange @endif">{{ $vehicle->tachograph->format('j.n.Y') }}</span>
                    </p>
                @elseif($vehicle->type === \App\Enums\VehicleType::WORK->value && $vehicle->insurance)
                    <p class="font-normal text-gray-700 dark:text-gray-400">
                        ...
                        <span class="badge @if($vehicle->insurance->diffInDays(now()) > -14) badge-red @elseif($vehicle->insurance->diffInDays(now()) > -60) badge-orange @endif">{{ $vehicle->insurance->format('j.n.Y') }}</span>
                    </p>
                @endif
            </div>
        </a>
